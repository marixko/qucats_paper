...
from utils.preprocessing import prep_wise, flag_observation
from utils.correct_extinction import correction
from utils.metrics import Odds, Q
from settings.columns import aper, broad, narrow, splus, error_splus, wise, galex, calculate_colors, calculate_ratio

def Process_Final(data, feature_list:list, Scaler_1, Scaler_2, aper=aper):
    ...
    flag_obs = False
    if flag_obs: data = flag_observation(data)
    ...
    # Calculate colors and/or ratios
    colors = [feat for feat in feature_list if ('-' in feat)]
    ratios = [feat for feat in feature_list if ('/' in feat)]
    ...
    mask = data[feature_list].isna().reset_index(drop=True)
    features[mask] = 0
...
def FinalPredict(Model:dict, Testing_Dataframe, Testing_Data_Features, Num_Samples=200):
    ...
    Result_DF = Result_DF.astype({'ID': str, 'RA': np.float64, 'DEC': np.float64, 'z_bmdn_peak': np.float32,
                                  'zphot_2.5p': np.float32, 'zphot_16p': np.float32,
                                  'zphot_84p': np.float32, 'zphot_97.5p': np.float32,
                                  'n_peaks_bmdn': np.float32, 'zphot_second_peak': np.float32,
