...
```{r}
saveRDS(pred_mean_broad, file.path(model_path, "pred_mean_broad+GALEX+WISE+flags.rds"))
```
...
```{r}  
write.csv(df, file.path(model_path, "mean_importances_broad+GALEX+WISE+flags.csv"))
```
...
```{r}   
write.csv(cov_broad, file.path(model_path, "pit_broad+GALEX+WISE+flags.csv"))
```
...
```{r}
saveRDS(pred_mean_narrow, file.path(model_path, "pred_mean_broad+GALEX+WISE+narrow+flags.rds"))
```
...
```{r}
write.csv(cov_narrow, file.path(model_path, "pit_broad+GALEX+WISE+narrow+flags.csv"))
```
...
```{r}  
write.csv(df, file.path(model_path, "mean_importances_broad+GALEX+WISE+narrow+flags.csv"))
```
...
colors_broad <- read.csv(file.path(bnn_path,"bnn_BWG_flag.csv"))
colors_narrow <- read.csv(file.path(bnn_path,"bnn_BNWG_flag.csv"))
```
...
dens_matrix_b = matrix(unlist(dens_broad),  nrow = length(dens_broad), byrow=TRUE)
dens_matrix_n = matrix(unlist(dens_narrow),  nrow = length(dens_narrow), byrow=TRUE)
```
