...
def sampling(Model:dict, Testing_Data, Testing_Data_Features, Num_Samples=200, Output_PDFs=True, aper=aper):

    Folds = Model.keys()
    print(f"# Predicting for {len(Folds)} folds")
    x = np.linspace(0, 5, 5000, endpoint=True)
    
    Testing_Data_Target = Testing_Data['Z'].values
    Result_DF = pd.DataFrame()
    Result_DF['SDSS_NAME'] = Testing_Data['SDSS_NAME'].values
    Result_DF['ID_SPLUS'] = Testing_Data['ID'].values
    Result_DF['RA'] = Testing_Data['RA_1'].values
    Result_DF['DEC'] = Testing_Data['DEC_1'].values
    Result_DF['r_'+aper] = Testing_Data['r_'+aper].values
